...
| FD002 | 6 Regimes | K-Means + Hybrid | **26.3** | Good |
...
::: {.column width="50%"}
![Sensor Degradation Pattern](images/FD001 RUL SENSOR.png){width="60%"}

*Multiple engines show the same "death curve" pattern as they approach failure.*
:::
:::

---

## 📊 Survival Analysis: Cox Proportional Hazards {.smaller}
...
![Risk Factors Analysis](images/FD001 Risk factors.png){width="80%"}
...
## 🔮 The Oracle: Weibull AFT Model

**Surpassing the Cox Model:**
Cox PH tells us *risk* (who dies first), but it can't predict *when*.
We need a model that output **time**.

**Why Weibull?**
- It's the standard engineering curve for "wear-out".
- It assumes engines age like a tire losing tread.

```r
# The Oracle: Accelerated Failure Time (AFT) Model
# We tell the model: "Predict the exact cycle of death"
...
**Method:** Principal Component Analysis (PCA) reduces dimensionality while preserving signal.

```r
# Compress all sensor data into one single "Health Score"
pca_result <- prcomp(sensors_only, scale. = TRUE)
data$Health_Index <- pca_result$x[, 1]  # First Principal Component
```

**Interpretation:**
- **Health Index increases** = Degradation severity increases.
- **Single Metric Monitoring**: Replaces complex multi-sensor dashboards.
...
![Top 15 Sensors Driving Engine Failure](images/FD001 top 15 sensors driving engine failure.png){width="85%"}

**Key Insight:** Rolling means of s11, s12, s4 dominate — consistent with Cox analysis.
...
::: {.column width="50%"}
![Model Performance Metrics](images/FD001 engine prediction performance.png){width="85%"}
:::
:::
...
::: {.columns}
::: {.column width="50%"}
![FD001 Predicted vs Actual RUL](images/FD001 FINAL PREDICTION.png){width="85%"}
:::

::: {.column width="50%"}
![Final Accuracy Check](images/FD001 final accuracy check.png){width="85%"}
:::
:::
...
::: {.columns}
::: {.column width="65%"}
![Survival Analysis: Kaplan-Meier Curves](images/FD003FLIGHT CYCLES.png){width="90%"}
:::
...
## The Unified Pipeline Architecture {.scrollable}

```
┌─────────────────────────────────────────────────────────────────────┐
│                         RAW SENSOR DATA                              │
│               (21 sensors × 248 engines × 200+ cycles)               │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  STEP 1: REGIME RECOGNITION                                         │
│  K-Means (k=6) → Identify operating condition from settings          │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  STEP 2: REGIME NORMALIZATION                                        │
│  Z-Score per Regime → Remove altitude/speed effects                  │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  STEP 3: FAULT CLASSIFICATION                                        │
│  Random Forest → Diagnose failure mechanism (Fault 0 vs Fault 1)     │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  STEP 4: FEATURE ENGINEERING                                         │
│  Rolling Statistics → Trend and volatility metrics                   │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  STEP 5: ENSEMBLE REGRESSION                                         │
│  XGBoost → Predict RUL using sensors + regime + fault context        │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  STEP 6: PHYSICS SMOOTHING                                           │
│  EMA Filter → Enforce smooth degradation for deployment              │
└─────────────────────────────────────────────────────────────────────┘
```
...
- **GitHub MLOps:** [turbofan-predictive-maintenance-mlops](https://github.com/AymenMB/turbofan-predictive-maintenance-mlops)
